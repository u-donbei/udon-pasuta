...
public abstract class Block extends GameObject {
    ...
    public Optional<PushBackDirection> pushBack(GameCharacter target) {
        ...
        if (top < 0 && top > -13 && top != -1 && !isTopObject) {
            target.moveY(top);
            return Optional.of(PushBackDirection.TOP);
        } else if (left < 0 && left > -13 && left != -1 && !isLeftObject) {
            target.moveX(left);
            return Optional.of(PushBackDirection.LEFT);
        } else if (bottom > 0 && bottom < 13 && bottom != 1 && !isBottomObject) {
            target.moveY(bottom);
            return Optional.of(PushBackDirection.BOTTOM);
        } else if (right > 0 && right < 13 && bottom != 1 && !isRightObject) {
            target.moveX(right);
